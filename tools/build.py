...
def BuildOneConfig(options, target, target_os, mode, arch, override_tools):
  ...
  if HOST_OS == 'macos':
    ...
    project_file = 'dusk.xcodeproj'
    if os.path.exists('dusk-%s.gyp' % CurrentDirectoryBaseName()):
      project_file = 'dusk-%s.xcodeproj' % CurrentDirectoryBaseName()
    args = ['xcodebuild',
            '-project',
            project_file,
            '-target',
    ...
    project_file = 'dusk.sln'
    if os.path.exists('dusk-%s.gyp' % CurrentDirectoryBaseName()):
      project_file = 'dusk-%s.sln' % CurrentDirectoryBaseName()
